 ...
 *
 * `client_id` is supplied by whoever begins the authorization request, so it
 * decides where this server sends a request — the specification calls out
 * SSRF here by name.
...
export const isFetchableOrigin = (url) => {
  ...
  // Any IP literal, loopback or not: none of them are valid client identifiers
  ...
  // ponytail: a domain name is trusted without resolving it, so one pointing
  // at an internal address still passes. The specification suggests resolving
  // first and rejecting addresses in the loopback range; doing that safely
  // also means connecting to the address that was checked, which needs a
  // custom dispatcher and is a larger change than this.
  return true;
};
