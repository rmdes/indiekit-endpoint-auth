...
export const getClientInformation = async (clientId) => {
  ...
  // Per IndieAuth spec, localhost client IDs should not be fetched
  // since the auth server cannot reach the client's local machine
  const clientUrl = new URL(clientId);
  if (clientUrl.hostname === "localhost" || clientUrl.hostname === "127.0.0.1") {
    return client;
  }
  ...
  try {
    ...
    // Network error (e.g., DNS failure, unreachable host)
    return client;
  }
